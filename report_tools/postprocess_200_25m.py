...
def extract_row(summary_path: Path, results_root: Path) -> dict[str, Any]:
    ...
    row.update({
        ...
        "spanning_radius_lr_m": finite_float(topo.get("spanning_radius_lr_m")),
        "spanning_radius_tb_m": finite_float(topo.get("spanning_radius_tb_m")),
    ...
    if isinstance(spectrum, list):
        for item in spectrum:
            ...
            if np.isfinite(q) and np.isfinite(dq):
                row[q_key(q)] = dq
                q_pairs.append((q, dq))
    if q_pairs:
        q_pairs.sort()
    ...
    if np.isfinite(area) and area > 0:
        ...
        row.update({
            ...
            "boundary_complexity_norm": row["boundary_complexity_index"] / sqrt_area,
        })
